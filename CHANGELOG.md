...
- Phone pass: body text 16px instead of 14px, larger section titles, role titles, and metric numbers; frameless cards, plays, tracks, and folds separated by hairlines, no backdrop blur except the header; 12px between neighbouring targets and a 44px header mark; fold rows without thumbnails and with short labels; the first play and both GTM tracks open by default; the playbook index as rows, the toolkit as four lines of names, a full-width contact action, inline note labels, shorter metric labels.
